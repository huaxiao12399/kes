...
export default async function handler(
  ...
  try {
    ...
    // 解析月份参数 (格式: YYYY-MM)
    const year = parseInt(month as string ? (month as string).split('-')[0] : new Date().getFullYear().toString());
    const monthNum = parseInt(month as string ? (month as string).split('-')[1] : (new Date().getMonth() + 1).toString());
